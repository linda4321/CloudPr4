...
import com.google.devrel.training.conference.form.*;
...
public class ConferenceApi {
    ...
    public Collection<Conference> getConferencesToAttend(final User user)
        ...
        ArrayList<Key<Conference>> keysToAttend = new ArrayList<>();

        for (String keyStrToAttend : keyStringsToAttend) {
            keysToAttend.add(Key.<Conference>create(keyStrToAttend));
        }
        ...
        WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>() {
            public WrappedBoolean run() {
                try {
                    ...
                    // Has the user already registered to attend this conference?
                    if (profile.getConferenceKeysToAttend().contains(websafeConferenceKey)) {
                        profile.deleteFromConferenceKeysToAttend(websafeConferenceKey);
                        conference.giveBackSeats(1);
                        ofy().save().entities(profile, conference).now();
                        return new WrappedBoolean(true, "Successfully unregistered");

                    } else
                        return new WrappedBoolean(false, "Error while unregistering. Maybe you are not registered to this conference");
                } catch (Exception e) {
                    return new WrappedBoolean(false, "Unknown exception");
                }
            }
